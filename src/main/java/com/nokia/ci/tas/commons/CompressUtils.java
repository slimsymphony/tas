...
/**
 * Util for compress and decompress data/String.
 * 
 * @author Frank Wang
 * @since Aug 21, 2012
 */
public class CompressUtils {
	 ...
	 */
	public static CompressType DefaultCompressType = CompressType.QUICKLZ;

	/**
	 * Compress byte array with assigned algorithm.
	 * 
	 * @param input data to be compressed
	 * @param compressType algorithm
	 * @return
	 * @throws Exception
	 * @since Aug 21, 2012
	 */
	public static byte[] compress( byte[] input, CompressType compressType ) throws Exception {
		if ( compressType == null )
			compressType = DefaultCompressType;
		switch ( compressType ) {
			case ZLIB:
				return ZLibCompress.compress( input );
			case GZIP:
	...
	/**
	 * Decompress byte array with assigned algorithm.
	 * @param input
	 * @param compressType
	 * @return
	 * @throws Exception
	 * @since Aug 21, 2012
	 */
	public static byte[] decompress( byte[] input, CompressType compressType ) throws Exception {
		if ( compressType == null )
			compressType = DefaultCompressType;
		switch ( compressType ) {
			case ZLIB:
				return ZLibCompress.decompress( input );
			case GZIP:
				return GZipCompressor.decompress( input );
			default:
	...
	/**
	 * Compress String with assigned algorithm. 
	 * 
	 * @param input
	 * @param compressType
	 * @return
	 ...
	 * @since Aug 21, 2012
	 */
	public static String compress( String input, CompressType compressType ) throws Exception {
		if ( input == null )
			return input;
		return new String( compress( input.getBytes( "ISO-8859-1" ), compressType ), "ISO-8859-1" );
	}

	/**
	 * Deompress String with assigned algorithm.
	 *  
	 * @param input
	 * @param compressType
	 * @return
	 ...
	 * @since Aug 21, 2012
	 */
	public static String decompress( String input, CompressType compressType ) throws Exception {
		if ( input == null )
			return input;
		return new String( decompress( input.getBytes( "ISO-8859-1" ), compressType ), "ISO-8859-1" );
	}
}

/**
 * QuickLz compress algorithm implementation.
 * 
 * @author Frank Wang
 * @since Aug 21, 2012
 */
...
/**
 * Gzip compress algorithm implementation.
 * 
 * @author Frank Wang
 * @since Aug 21, 2012
 */
final class GZipCompressor {
	...
	public static byte[] compress( byte[] input ) throws IOException {
		...
		try {
			...
			int count;
			while ( ( count = bai.read( buf, 0, BUFFER_SIZE ) ) != -1 ) {
				gos.write( buf, 0, count );
			}
			gos.flush();
		} finally {
			MonitorUtils.close( gos );
		}
...
/**
 * Zlib compress algorithm implementation.
 * 
 * @author Frank Wang
 * @since Aug 21, 2012
 */
final class ZLibCompress {
	...
	public static byte[] decompress( byte[] compressedData ) {
		...
		Inflater decom = new Inflater();
		decom.setInput( compressedData );
		byte[] buf = new byte[1024];
		ByteArrayOutputStream bos = new ByteArrayOutputStream();
		while ( !decom.finished() ) {
			int count;
			try {
				count = decom.inflate( buf );
				bos.write( buf, 0, count );
